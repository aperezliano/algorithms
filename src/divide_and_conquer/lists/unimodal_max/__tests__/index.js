...
it.each([[[1]], [[2]], [[null]]])('returns the only element of an array of a single element: %s', (array) => {
  expect(unimodalMax(array)).toEqual(array[0]);
});

it.each([[[1, 2]], [[4, 3]]])('returns the max of an array of 2 elements: %s', (array) => {
...
])('returns the max of an array of multiple elements: %s', (array, expected) => {
